...
void init_volume(volume_info_t *volume, const char *path, volume_type type, int volume_id)
{
    ...
    snprintf(volume->bitmap_path, MAX_PATH_LENGTH, "%s/bitmap_%d.bin", path, volume_id);
...
void load_or_create_superblock(const char *path, superblock_t *sb)
{
    ...
    if (!file)
    {
        ...
        {                                                            // Initialize each volume info
            init_volume(&sb->volumes[i], "/default/path", LOCAL, i); // Default initialization
        }
...
void create_volume_files_local(int i, superblock_t *sb)
{
    ...
    FILE *merkle_file = fopen(sb->volumes[0].merkle_path, "wb+");
    if (!merkle_file)
    {
        printf("Merkle file not found, creating a new one.\n");
        MerkleTree *merkle_tree = initialize_merkle_tree_for_volume(sb->volumes[0].volume_path);
        save_merkle_tree_to_file(merkle_tree, sb->volumes[0].merkle_path);
    }
    else
    {
        fclose(merkle_file);
...
void write_volume_block(const char *volume_id, int block_index, const void *buf)
{
    ...
    update_merkle_node_for_block(volume_id, block_index, buf);
}
